...
OPENAI_API_URL = None
FINETUNED_URL  = None        # finetuned model url

OPENAI_API_KEY = None           # you should alway setup openai api key for evaluation
GEMINI_API_KEY = None   # no need when using google cloud
QWEN_API_KEY   = None
CLAUDE_API_KEY = None
...
class OpenAIModel(BasicModel):
    def __init__(self):
        super().__init__()
        self.client = None # Client will be created per request with args
    ...
    def get_model_response(self, messages: List[Dict], model_name: str, **args) -> Tuple[bool, str]:
        ...
        if not api_key:
            raise ValueError(
                "api_key must be passed in arguments (env vars ignored)."
            )
        ...
        try:
            ...
            # Add top_p only if it's not None/1.0
            ...
            try:
                ...
                error_msg = str(e)
                # Retry without top_p if not supported
                if "top_p" in error_msg and "not supported" in error_msg:
                    print(f"Retrying without top_p due to error: {e}")
                    kwargs.pop("top_p", None)
                    response = client.chat.completions.create(**kwargs)
                else:
                    print("error when getting response:", e)
                    raise e
            ...
            print("error when getting response1:", e)
            return False, str(e)
...
class ClaudeModel(BasicModel):
    ...
    def get_model_response(self, messages: List[Dict], model_name: str, **args) -> Tuple[bool, str]:
        ...
        if not client:
             return False, "Claude API key not found."
...
class GeminiModel(BasicModel):
    ...
    def get_model_response(self, messages: List[Dict], model_name: str, **args) -> Tuple[bool, str]:
        ...
        url = f"https://generativelanguage.googleapis.com/v1beta/models/{model_name}-latest:generateContent?key={api_key}"
...
class APIModel(object):
    ...
    def inference(self, model_id: str, messages: List[Dict], args: Dict) -> Tuple[bool, str]:
        ...
        if model_provider not in self.models:
            self.models[model_provider] = OpenAIModel()
            print(f"Model not found: {model_provider} ({model_name}), using OpenAIModel as default.")
            # return OpenAIModel()
        ...
        logger.info(f"Constructed prompt for {model_provider} ({model_name}): {prompt}")
        resp = model.get_model_response(prompt, model_name, **args)
        
        return resp
        
model = APIModel(base_url=None, api=None)
