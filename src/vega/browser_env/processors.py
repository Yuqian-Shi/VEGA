...
from vega.browser_env.constants import (
    ASCII_CHARSET,
...
class TextObervationProcessor(ObservationProcessor):
    ...
    def fetch_browser_info(
        ...
        # Note: devicePixelRatio may not be 1.0 on systems with DPI scaling (e.g., Windows)
        # We use the actual ratio for coordinate calculations rather than asserting it must be 1.0
    ...
    def fetch_page_html(
        ...
        # Create a mapping from node index to bounds
        node_idx_to_bounds = {}
        for i, node_idx in enumerate(layout["nodeIndex"]):
            node_idx_to_bounds[node_idx] = layout["bounds"][i]
        ...
        for node_idx in range(len(nodes["nodeName"])):
            ...
            if cur_node["parentId"] == "-1":
                ...
                if node_idx in node_idx_to_bounds:
                    cur_node["union_bound"] = node_idx_to_bounds[node_idx]
                else:
    ...
    def fetch_page_accessibility_tree(
        ...
        # Build backend_node_id_to_bounds map
        dom_tree_info = info["DOMTree"]
        document = dom_tree_info["documents"][0]
        nodes = document["nodes"]
        layout = document["layout"]

        backend_node_id_to_bounds = {}
        for i, node_idx in enumerate(layout["nodeIndex"]):
            backend_node_id = str(nodes["backendNodeId"][node_idx])
            backend_node_id_to_bounds[backend_node_id] = layout["bounds"][i]
        ...
        for cursor, node in enumerate(accessibility_tree):
            ...
            if node["role"]["value"] == "RootWebArea":
                ...
                if backend_node_id in backend_node_id_to_bounds:
                    node["union_bound"] = backend_node_id_to_bounds[backend_node_id]
                else:
                    node["union_bound"] = None
    ...
    def get_element_center(self, element_id: Union[int, str]) -> tuple[float, float]:
...
class ImageObservationProcessor(ObservationProcessor):
    ...
    def get_element_center(self, element_id: Union[int, str]) -> tuple[float, float]:
        ...
        center_x, center_y, width, height = self.som_id_info[str(element_id)]
        return (
            center_x / self.viewport_size["width"],
            center_y / self.viewport_size["height"],
        )
...
class TextObervationProcessorWebRL(TextObervationProcessor):
    ...
    def get_element_center(self, element_id: Union[int, str], page: Page=None) -> tuple[float, float]:
