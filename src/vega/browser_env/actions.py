...
from vega.browser_env.processors import ObservationProcessor
...
from vega.browser_env.processors import (
        ...
        match action["action_type"]:
            ...
            case ActionTypes.TYPE:
                ...
                if isinstance(text, list):
                    text = "".join([_id2key[i] for i in text])
            ...
            case ActionTypes.SEARCH:
                ...
                if isinstance(text, list):
                    text = "".join([_id2key[i] for i in text])
                action_str = f"search [{element_id}] [{text}]"
            case ActionTypes.SELECT_DROPDOWN_OPTION:
                argument = action.get("argument", "")
                action_str = f"select_dropdown_option [{element_id}] [{argument}]"
            case ActionTypes.SCROLL:
                action_str = f"scroll [{action['direction']}]"
            case ActionTypes.KEY_PRESS:
                action_str = f"press [{action['key_comb']}]"
...
def is_equivalent(a: Action, b: Action) -> bool:
    ...
    match (a["action_type"]):
        ...
        case ActionTypes.SELECT_DROPDOWN_OPTION:
            if a["element_id"] and b["element_id"]:
                return a["element_id"] == b["element_id"] and a.get("argument") == b.get("argument")
            else:
                return False
        case ActionTypes.SEARCH:
            if a["element_id"] and b["element_id"]:
                return a["element_id"] == b["element_id"] and a.get("text") == b.get("text")
            else:
                return False
        case ActionTypes.STOP:
...
def create_random_action() -> Action:
    ...
    return {
        ...
        "element_id": int(np.random.randint(MAX_ELEMENT_ID)),
        "key_comb": "+".join(
...
def create_id_based_action(action_str: str) -> Action:
    ...
    match action:
        ...
        case "type":
            ...
            match = re.search(
                r"type ?\[(\d+)\] ?\[(.+?)\]", action_str, flags=re.DOTALL
            )
            if not match:
            ...
            # Check for enter flag in the remaining string
            # We look for the first occurrence of [0] or [1] after the text
            remaining = action_str[match.end():]
            enter_flag = "1" # Default
            
            flag_match = re.search(r"\[(\d)\]", remaining)
            if flag_match:
                enter_flag = flag_match.group(1)
        ...
        case "stop":  # stop answer
            match = re.search(r"stop ?\[(.+)\]", action_str, flags=re.DOTALL)
    ...
    match operation:
        case "do":
            ...
            match action_type:
                ...
                case "click":
                    element_id = str(action["kwargs"]["element"])
                    return create_click_action(element_id=element_id)
                case "type":
                    element_id = str(action["kwargs"]["element"])
