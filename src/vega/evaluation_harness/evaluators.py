...
class StringEvaluator(Evaluator):
    ...
    def fuzzy_match(
        ...
        return llm_fuzzy_match(pred, ref, intent, model_cfg, result_dir, task_id)
    ...
    def ua_match(
        ...
        return llm_ua_match(pred, ref, intent, model_cfg, result_dir, task_id)
            ...
            match approach:
                ...
                case "must_include":
                    ...
                    for v in must_include:
                        ...
                        score *= any(
                            [
                                self.must_include(ref=v_or, pred=pred)
                                for v_or in value_or
                            ]
                        )
                case "must_exclude":
                    must_exclude = value
                    assert isinstance(must_exclude, list)
                    for v in must_exclude:
                        value_or = v.split(" |OR| ")
                        score *= any(
                            [
                                self.must_exclude(ref=v_or, pred=pred)
                                for v_or in value_or
                            ]
                        )
                case "fuzzy_match":
                    fuzzy_match = value
                    assert isinstance(fuzzy_match, list)
                    for v in fuzzy_match:
                        value_or = v.split(" |OR| ")
                        score *= any(
                            [
                                self.fuzzy_match(
                ...
                case "ua_match":
                    ua_match = value
                    assert isinstance(ua_match, list)
                    for v in ua_match:
                        value_or = v.split(" |OR| ")
                        score *= any(
                            [
...
class HTMLContentExactEvaluator(Evaluator):
    ...
    def fuzzy_match(
        ...
        return llm_fuzzy_match(pred, ref, intent, model_cfg, result_dir)
