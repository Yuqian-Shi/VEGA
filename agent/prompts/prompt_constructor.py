...
class PromptConstructor(object):
    ...
    def get_lm_api_input(
        ...
        if "openai" in self.lm_config.provider or self.lm_config.provider == "api":
...
class CoTPromptConstructor(PromptConstructor):
    ...
    def construct(
        ...
        max_obs_length = self.lm_config.gen_config["max_obs_length"]
        if max_obs_length:
            if self.lm_config.provider in ["google", "api", "finetune","openai"]:
                # print(f"NOTE: provider is {self.lm_config.provider}, so we use characters instead of tokens for max_obs_length.")
                obs = obs[:max_obs_length]
            else:
                obs = self.tokenizer.decode(self.tokenizer.encode(obs)[:max_obs_length])  # type: ignore[arg-type]
...
class MultimodalCoTPromptConstructor(CoTPromptConstructor):
    ...
    def construct(
        ...
        max_obs_length = self.lm_config.gen_config["max_obs_length"]
        if max_obs_length:
            if self.lm_config.provider in ["google", "api", "finetune","openai","api"]:
                obs = obs[:max_obs_length]
            else:
        ...
        if max_obs_length:
            if self.lm_config.provider == "google":
                ...
                try:
                    ...
                    # logger.info("NOTE: There is no available tokenizer, so we use characters instead of tokens for max_obs_length.")
                    obs = obs[:max_obs_length]
